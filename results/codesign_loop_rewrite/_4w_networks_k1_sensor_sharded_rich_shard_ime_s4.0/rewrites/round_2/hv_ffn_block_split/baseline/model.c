...
static void dispatch_ffn_block_1(model_ffn_block_state_t *s) {
    unsigned long _s = rdcycle();
    parallel_linear_s8(s->pool, buf_ffn_block_ln, ffn_block_fc1_weight_q_scalar, ffn_block_fc1_bias_q_scalar, buf_ffn_block_fc1, 128, 256, 1024, 0, 0, 0, 1571127057, 10, -128, 127);
    unsigned long _e = rdcycle();
    printf("=== MODELBLASTER_INSPECT_BEGIN [fc1] === scale=%g dtype=i8 n=%d\n", 0.021946745f, 131072);
    { const int8_t *_p = (const int8_t *)buf_ffn_block_fc1;
      for (int _i = 0; _i < 131072; _i++) printf("%g\n", (double)(float)_p[_i]); }
    printf("=== MODELBLASTER_INSPECT_END [fc1] ===\n");
    int slot = (n_++) % MODEL_FFN_BLOCK_OP_COUNT;
    records_[slot].dispatch_id = 1;
    records_[slot].name   = "fc1";
    records_[slot].op     = "linear_s8";
    records_[slot].shape  = "M=128;K=256;N=1024";
    records_[slot].cycles = _e - _s;
}
