...
ap.add_argument("--replay-actions", default=None,
                help="npy of recorded applied_actions. Replays them instead of running "
                     "the policy. The harness is not run-to-run deterministic because "
                     "POLICY inference is not; the physics given an identical action "
                     "stream is. Replaying therefore reproduces a stored rollout exactly, "
                     "which is the only way to render a mid-episode frame that matches a "
                     "trace already on disk.")
ap.add_argument("--dump-frame-tick", type=int, default=None,
                help="Save the rendered RGB frame at this tick and exit the episode.")
...
_REPLAY = np.load(args.replay_actions) if args.replay_actions else None
if _REPLAY is not None:
    print(f"[replay] {len(_REPLAY)} recorded actions; policy output ignored", flush=True)
...
for k in range(args.n):
    ...
    while t < MAX_TICKS:
        ...
        if cur_raw is None:
            ...
            fresh = (t in apply_at)
        if _REPLAY is not None:
            raw7 = _REPLAY[min(t, len(_REPLAY) - 1)]
        ...
        if args.dump_frame_tick is not None and t == args.dump_frame_tick:
            import PIL.Image as _PI
            _PI.fromarray(render_now()).save(f"{run}/frame_t{t:04d}.png")
            print(f"[dump] wrote {run}/frame_t{t:04d}.png at tick {t}", flush=True)
