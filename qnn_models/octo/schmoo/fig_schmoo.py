...
Colour   the observation age / cadence CP-SAT actually achieves in that cell. Only the
         66 SOLVED cells have a value; the rest are blank by construction, not missing
         data.
...
Box      the 39 distinct operating points being run in the wide E2E sweep.
...
warm = json.load(open(HERE / "grid_warm.json"))
cells = warm["cells"]
...
arms = next((c for c in (HERE / "arms.tsv", HERE.parent / "g5grid" / "arms.tsv")
             if c.exists()), HERE / "arms.tsv")
if arms.exists():
    for ln in open(arms):
