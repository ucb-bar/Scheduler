...
#              ^ solo #0     ^ solo #1     ^ both harts
SOLO0, SOLO1, PAIR = 0, 1, 2
...
def _two_hart_workload(pt_a, pt_b):
    ...
    """
    machines, combos = build_machine_combinations({"CPU_P": 2}, mode="shard")
    assert machines == ["CPU_P#0", "CPU_P#1"], machines
    assert combos == [["CPU_P#0"], ["CPU_P#1"], ["CPU_P#0", "CPU_P#1"]], combos
...
def test_payload_lists_every_machine_a_combination_occupies():
    ...
    assert payload["combo_machines"] == [[0], [1], [0, 1]], payload["combo_machines"]
    ...
    assert payload["first_machine"] == [0, 1, 0], payload["first_machine"]
...
def test_integerize_lets_sibling_harts_run_concurrently():
    ...
    w = _two_hart_workload([10.0, INF, INF], [INF, 10.0, INF])
    ctx = DecoderContext(w)
...
def test_integerize_serialises_a_two_core_op_against_a_singleton():
    ...
    w = _two_hart_workload([INF, INF, 10.0], [INF, 10.0, INF])
    ctx = DecoderContext(w)
    dur_int = build_payload(ctx, time_limit=1.0)["dur"]
    alpha = np.zeros((2, 3))
    alpha[0, PAIR] = 1.0
...
def test_model_forbids_a_two_core_op_overlapping_a_singleton():
    ...
    for solo, name in ((SOLO0, "CPU_P#0"), (SOLO1, "CPU_P#1")):
        pt_b = [INF, INF, INF]
        pt_b[solo] = 10.0
        w = _two_hart_workload([INF, INF, 10.0], pt_b)
        t, alpha, all_end = _solve(w)
